...
import (
	...
	"github.com/sottey/revinder/consumers/revinder_memory_consumer/internal/consumer"
	"github.com/sottey/revinder/consumers/revinder_memory_consumer/internal/memory"
)
...
func run() error {
	var (
		...
		memoryFile = flag.String("memory-file", envDefault("REVINDER_MEMORY_FILE", ""), "memory JSONL output file")
	...
	if *configPath != "" {
		...
		if cfg.MemoryFile != "" && !visited["memory-file"] {
	...
	if *memoryFile == "" {
		return fmt.Errorf("memory_file is required")
	}

	logger := slog.New(slog.NewJSONHandler(os.Stdout, nil))
	logger.Info("consumer_started", "bridge_url", *bridgeURL, "consumer", "memory", "memory_file", *memoryFile, "interval", interval.String(), "once", *once)

	ctx, stop := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer stop()

	c := consumer.New(bridge.NewClient(*bridgeURL, *token), memory.New(*memoryFile), logger)
	if *once {
